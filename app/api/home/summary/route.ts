...
export async function GET() {
	try {
		...
		const [user, notifications, unreadCount, unreadMessages, circlesRaw, pendingBorrowRaw, openItemRequestsRaw, recentThreads] =
			await Promise.all([
		...
		const [avatarUrlMap, itemUrlMap] = await Promise.all([
			circleAvatarPaths.length ? getSignedUrls(circleAvatarPaths, 'avatars').catch(() => new Map()) : new Map<string, string>(),
			itemImagePaths.length ? getSignedUrls(itemImagePaths, 'items').catch(() => new Map()) : new Map<string, string>(),
		]);
